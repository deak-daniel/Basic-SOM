namespace UnsupervisedPerceptron
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int index = 0; List<KohonenNeuron> neurons = new List<KohonenNeuron>();
            ...
            while (index < NUMBER_OF_TRAINING_CYCLES)
            {
                in1 = Math.Round(rnd.NextDouble(), 2);
                in2 = Math.Round(rnd.NextDouble(), 2);
                for (int i = 0; i < neurons.Count; i++)
                {
                    neurons[i].Initialize(in1, in2);
                    neurons[i].StartEval();
                }
                ...
                double minDeg = neurons.Max(y => y.Output); // selecting the vector closest (smallest degree value between it and the input vector)
                                                            // to the input vector
                KohonenNeuron winnerNeuron = neurons[neurons.IndexOf(neurons.FirstOrDefault(x => x.Output == minDeg))];


                winnerNeuron.TrainWeights();
                Console.WriteLine($"\tNeuron: {winnerNeuron.ID} output: {winnerNeuron.WeightVectorLength}, weight_1: {winnerNeuron.Weight_1}, weight_2: {winnerNeuron.Weight_2} ");
    ...
    public class KohonenNeuron
    {
        ...
        public void Initialize(double? input1 = null, double? input2 = null)
        {
        ...
        public void StartEval()
        {
            ...
            this.Output = Math.Round((this.Normalized_Input_1 * this.Normalized_Weight_1) + (this.Normalized_Input_2 * this.Normalized_Weight_2), 2);
